...
export default function AppointmentDetailScreen() {
  ...
  const hasQR      = isApproved && !!appointment.qrToken && appointment.qrToken.length > 0;
   ...
   <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      >
        ...
        >
          ...
          {/* ── Status banner ── */}
          <View style={[styles.statusBanner, { backgroundColor: statusBg(appointment.status) }]}>
            <Text style={[styles.statusBannerText, { color: statusColor(appointment.status) }]}>
          ...
          {isRejected && appointment.rejectionReason && (
            <View style={styles.rejectionBanner}>
              ...
              <Text style={styles.rejectionBody}>{appointment.rejectionReason}</Text>
            </View>
          )}
          ...
          <View style={styles.ticketCard}>
            ...
            {/* ── QR section ── */}
            {hasQR ? (
              /* APPROVED + valid qrToken → live QR */
              ...
              /* PENDING → blurred placeholder */
              <View style={styles.qrSection}>
                <View style={styles.qrPendingOuter}>
                  <View style={styles.qrPendingOverlay}>
                    <Text style={styles.qrPendingIcon}>🔒</Text>
                    <Text style={styles.qrPendingText}>Awaiting Approval</Text>
                    <Text style={styles.qrPendingSubtext}>
                      QR will be available once approved
                    </Text>
                  </View>
                </View>
                <Text style={styles.ticketNumber}>
              ...
              /* REJECTED → no QR, show icon */
              <View style={styles.qrSection}>
                <View style={styles.qrRejectedOuter}>
                  <Text style={styles.qrRejectedIcon}>✕</Text>
                  <Text style={styles.qrRejectedText}>Entry Not Permitted</Text>
                </View>
              </View>
            )}
            ...
            {/* Detail rows */}
            <DetailRow icon="📅" label="DATE"    value={formatDate(appointment.appointmentDate)} />
            <DetailRow
              icon="👥"
              label="COMPANIONS"
              value={
            ...
            />
            <DetailRow icon="📄" label="REASON"  value={appointment.purposeOfVisit} />
            <DetailRow
              icon="📍"
              label="MEETING VENUE"
          ...
          <Text style={styles.disclaimer}>
            ⓘ Entry permitted only with valid identity proof matching visitor name.
          </Text>
...
function Header({
  ...
  return (
    <View style={styles.header}>
      <TouchableOpacity onPress={onBack} hitSlop={12} style={styles.headerBackBtn}>

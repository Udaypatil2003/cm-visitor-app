...
function mapBackendToAppointment(raw: any): Appointment {
  return {
    ...
    qrExpiresAt: raw.qrexpiresat ?? raw.qr_expires_at ?? getMidnight(raw.appointmentdate ?? raw.appointment_date),
...
async getAppointmentById(id: string): Promise<ApiResponse<Appointment>> {
  const response = await apiClient.get<any>(Endpoints.GET_APPOINTMENT_BY_ID(id));
  return {
    success: response.data.success,
    message: response.data.message ?? '',
...
async createAppointment(data: CreateAppointmentPayload): Promise<ApiResponse<Appointment>> {
  const response = await apiClient.post<any>(Endpoints.CREATE_APPOINTMENT, data);
  return {
    success: response.data.success,
    message: response.data.message ?? '',
    data: mapBackendToAppointment(response.data.appointment),
